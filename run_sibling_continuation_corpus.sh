...
if [[ "$policy" != "feedback" && "$policy" != "feedback_balanced" && "$policy" != "feedback_marginal" && "$policy" != "feedback_ucb" ]]; then
    echo "policy must be feedback, feedback_balanced, feedback_marginal, or feedback_ucb" >&2
    exit 6
fi
