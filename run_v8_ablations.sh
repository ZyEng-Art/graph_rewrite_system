...
launch() {
    ...
    nohup env CUDA_VISIBLE_DEVICES="$gpu" "$python_bin" "$workdir/train.py" \
        ...
        --readout-graph-input cached --learning-rate 5e-5 \
...
launch 5 paged_action_topnboundary_v8_w050 \
    --topn-boundary-weight 0.5
launch 6 paged_action_topnboundary_v8_w100 \
    --topn-boundary-weight 1.0
launch 7 paged_action_topnboundary_v8_w050_local1 \
    --topn-boundary-weight 0.5 --locality-positive-weight 1.0 \
