...
if [[ $# -ne 5 ]]; then
  echo "usage: $0 GPU_ID QASM_NAME DEPTH {off|neural|native|hybrid} OUTPUT_TAG" >&2
  exit 2
fi
...
case "$prefilter_mode" in
  off)
    extra_args+=(--preapply-fingerprint off)
    ;;
  neural)
    extra_args+=(
  ...
  hybrid)
    ...
    ;;
  *)
    echo "unknown mode: $prefilter_mode" >&2
    exit 2
    ;;
esac
